...
namespace lab11
{
    class CEmblem
    {
        ...
        private void Draw(Pen pen)
        {
            ...
            Rectangle cube = new Rectangle(X - 3 * Radius, Y - Radius, Radius * 2, Radius * 2);
            graphics.DrawRectangle(pen, cube);
            double r = (Radius);
            Point p1 = new Point(X + 3*(int)(r * Math.Cos(Math.PI / 6)), Y + 2*(int)(r * Math.PI / 6));
            Point p2 = new Point(X + 3*(int)(r * Math.Cos(Math.PI / 6)), Y - (int)(r * Math.PI / 3.2));
            Point p3 = new Point(X + (5/3)*(int)(r * Math.Cos(Math.PI / 18)), Y + (1/2)/(int)(r * Math.PI / 6));
